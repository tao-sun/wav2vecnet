...
def main():
    ...
    n_tasks += 1
    asc_datasets, asc_labels, asc_names, asc_label_list = load_asc_data(datapath + 'asc')
    ...
    n_tasks += 1
    dat_datasets, dat_labels, dat_names, dat_label_list = load_dat_data(datapath + 'dat')
    ...
    for epoch in range(1, Nepoch + 1):
        ...
        for id in tqdm(range(MAX_NUM_FILE * n_tasks), file=sys.stdout):
            ...
            labelData_tensor = torch.from_numpy(labelData)
            inputData_tensor = inputData_tensor.squeeze(-1)
            inputData_tensor, labelData_tensor = inputData_tensor.to(device), labelData_tensor.to(device)
            # GRADIENT DESCENT UPDATE
            pred = net.forward(inputData_tensor, task_id=ntask)
            loss = loss_fn(pred, labelData_tensor)
            ...
            current_numpy = loss.to('cpu').detach().numpy()
            # print(pred_numpy)
            # print(current_numpy)
            train_all[ntask][file_id] = current_numpy
            train_pred_all[ntask][file_id] = np.reshape(pred_numpy, [1, -1])
            train_label_all[ntask][file_id] = labelData
            # break

        str = "Train_Epo:%d " % (epoch)

        # COMPUTE ERRORS
        for ntask in range(n_tasks):
            str += "Mean_Loss:%.6f " % (np.mean(train_all[ntask][np.where(train_all[ntask])]))
            if error_type[ntask] == 1:  # MEAN CLASSIFICATION ERROR
                str += "MEAN_CLSIFCATION_ERR:%.6f " % (np.mean(1.0 * (np.argmax(np.vstack(train_pred_all[ntask]), axis=1) == np.argmax(
                ...
                str += "MEAN_EQL_ERR:%.6f " % (eer)

        print(str)

        if epoch % 25 > 0:  # VALIDATION LOOP EVERY 25 TRAINING EPOCHS
            continue
        torch.save(net.state_dict(), outfolder + "/loss_model.pt")
        ...
        for ntask in range(n_tasks):
            ...
            for id in tqdm(range(len(names[ntask]['val'])), file=sys.stdout):
                ...
                labelData = np.reshape(
                    1. * np.array([(l in labels[ntask]['val'][file_id]) for l in label_lists[ntask]]), (1, -1))
                ...
                pred_numpy = pred.to('cpu').detach().numpy()
                current_numpy = loss.to('cpu').detach().numpy()
                test_all[ntask][file_id] = current_numpy
                test_pred_all[ntask][file_id] = pred_numpy
        ...
        # COMPUTE ERRORS
        for ntask in range(n_tasks):
            str += "Mean_Loss:%.6f " % (np.mean(test_all[ntask][np.where(test_all[ntask])]))
            if error_type[ntask] == 1:  # CLASSIFICATION ERROR
                str += "MEAN_CLSIFCATION_ERR:%.6f " % (np.mean(1.0 * (np.argmax(np.vstack(test_pred_all[ntask]), axis=1) == np.argmax(
